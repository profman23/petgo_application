...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Admin: Get AR Balance data for all customers
  app.get('/api/admin/ar-balance', requireAdminAuth, async (req, res) => {
    try {
      console.log('🏦 Fetching AR Balance data...');
      
      // Get all customers with invoices
      const customersWithInvoices = await db.execute(sql`
        SELECT DISTINCT 
          gi.customer_name as customerName,
          gi.customer_phone as customerPhone,
          gi.customer_email as customerEmail
        FROM generated_invoices gi
        ORDER BY gi.customer_name
      `);

      // Calculate balance for each customer
      const arBalanceData = await Promise.all(
        customersWithInvoices.rows.map(async (customer: any) => {
          // Get total invoice amount for this customer
          const invoiceTotal = await db.execute(sql`
            SELECT COALESCE(SUM(CAST(final_total AS DECIMAL)), 0) as total
            FROM generated_invoices 
            WHERE customer_phone = ${customer.customerphone}
          `);

          // Get total payments for this customer
          const paymentsTotal = await db.execute(sql`
            SELECT COALESCE(SUM(CAST(ip.amount AS DECIMAL)), 0) as total
            FROM invoice_payments ip
            JOIN generated_invoices gi ON ip.booking_id = gi.booking_id
            WHERE gi.customer_phone = ${customer.customerphone}
          `);

          // Get total credit notes for this customer  
          const creditNotesTotal = await db.execute(sql`
            SELECT COALESCE(SUM(CAST(final_total AS DECIMAL)), 0) as total
            FROM credit_notes cn
            JOIN generated_invoices gi ON cn.invoice_number = gi.invoice_number
            WHERE gi.customer_phone = ${customer.customerphone}
          `);

          const invoiceAmount = parseFloat((invoiceTotal.rows[0] as any)?.total || 0);
          const paymentsAmount = parseFloat((paymentsTotal.rows[0] as any)?.total || 0);
          const creditNotesAmount = parseFloat((creditNotesTotal.rows[0] as any)?.total || 0);
          
          // Formula: Opening Balance + Invoice Value - Income Payment - Credit Note + Outgoing Payment
          // For now, assuming Opening Balance and Outgoing Payment are 0
          const balance = invoiceAmount - paymentsAmount - creditNotesAmount;
          ...
          return {
            ...
            invoiceTotal: invoiceAmount.toFixed(2),
            paymentsTotal: paymentsAmount.toFixed(2),
            creditNotesTotal: creditNotesAmount.toFixed(2)
          };
        })
      );

      // Filter out customers with zero balance (optional - show all for now)
      console.log(`✅ AR Balance calculated for ${arBalanceData.length} customers`);
  ...
  // Admin: Get detailed transactions for a specific customer
  app.get('/api/admin/ar-balance/transactions/:customerPhone', requireAdminAuth, async (req, res) => {
    try {
      const { customerPhone } = req.params;
      console.log(`🔍 Fetching transaction details for customer: ${customerPhone}`);
      ...
      const invoices = await db.execute(sql`
        SELECT 
          ...
          CAST(final_total AS DECIMAL) as amount,
          'Dr' as debitCredit,
          booking_id
        FROM generated_invoices 
      ...
      const payments = await db.execute(sql`
        SELECT 
          'Payment-' || ip.id as reference,
          'Payment' as type,
          ip.created_at::date as date,
          CAST(ip.amount AS DECIMAL) as amount,
          'Cr' as debitCredit,
          ip.booking_id
        FROM invoice_payments ip
        JOIN generated_invoices gi ON ip.booking_id = gi.booking_id
        WHERE gi.customer_phone = ${customerPhone}
      `);

      // Get all credit notes for this customer
      const creditNotes = await db.execute(sql`
        SELECT 
      ...
      const transactionsWithBalance = allTransactions.map((transaction: any) => {
