...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Admin: Get sales report data
  app.get('/api/admin/sales-report', requireAdminAuth, async (req, res) => {
    try {
      const bookings = await storage.getAllBookings();
      
      const salesData = await Promise.all(bookings.map(async (booking) => {
        const user = await storage.getUser(booking.userId);
        ...
        try {
          const invoiceStatus = await storage.getInvoiceStatus(booking.id);
          if (invoiceStatus?.isGenerated) {
            hasInvoice = true;
            const invoiceItems = await storage.getInvoiceItems(booking.id);
            
            if (invoiceItems && invoiceItems.length > 0) {
              const subtotal = invoiceItems.reduce((sum, item) => sum + parseFloat(item.total || '0'), 0);
              const taxAmount = subtotal * 0.15; // 15% tax
              const discountAmount = invoiceStatus.discountApplied ? subtotal * 0.1 : 0; // 10% discount
          ...
          console.log('No invoice found for booking:', booking.id);
        }
        
        return {
          id: booking.id,
          invoiceNumber: hasInvoice ? `INV-${booking.id}` : null,
          customerName: user?.name || 'Unknown',
          customerPhone: user?.phone || '',
          customerEmail: user?.email || '',
          appointmentDate: booking.scheduledDate || booking.createdAt,
          appointmentTime: booking.scheduledTime || '00:00',
          status: booking.status,
          serviceType: booking.serviceType || 'General Check Up',
          vetsVanCode: (driver as any)?.vetsvanCode || 'Unknown',
          vetsVanName: (driver as any)?.vetsvanName || 'Unknown',
          hasInvoice,
          invoiceDetails,
          items,
  ...
  // Get sales report data for admin dashboard
  app.get('/api/admin/sales-report', requireAdminAuth, async (req: any, res) => {
    try {
      console.log('Sales report called - starting data fetch...');
      
      // Get all bookings with invoice status
      const bookings = await storage.getAllBookings();
      ...
      for (const booking of bookings) {
        try {
          ...
          if (invoiceStatus && invoiceStatus.isGenerated) {
            invoiceItems = await storage.getInvoiceItems(booking.id);
            invoiceDetails = {
              subtotal: parseFloat(invoiceStatus.subtotal.toString()),
              taxAmount: parseFloat(invoiceStatus.taxAmount.toString()),
              discountAmount: parseFloat(invoiceStatus.discountAmount.toString()),
              finalTotal: parseFloat(invoiceStatus.finalTotal.toString()),
              generatedAt: invoiceStatus.generatedAt,
              generatedBy: invoiceStatus.generatedBy
            };
          }
          ...
          // Get VetsVan info
          const driver = await storage.getDriver(booking.vetsVanId);
          ...
          const salesRecord = {
            ...
            hasInvoice: !!invoiceStatus?.isGenerated,
            invoiceDetails,
            items: invoiceItems.map(item => ({
              description: item.description,
      ...
      console.log(`Sales report data processed successfully: ${salesData.length} records`);
      res.json(salesData);
