...
import { loginSchema, insertUserSchema, rideRequestSchema, registerSchema, otpVerificationSchema, insertOtpVerificationSchema, insertPaymentTransactionSchema, type PaymentTransaction, type PaymentMethod } from "@shared/schema";
import { ZodError } from "zod";
import { emailService } from "./emailService";
import bcrypt from 'bcrypt';
import { myfatoorahService } from "./myfatoorah";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ============== PAYMENT GATEWAY ROUTES (MyFatoorah) ==============

  // Get available payment methods
  app.get("/api/payments/methods", requireAuth, async (req, res) => {
    try {
      // First try to get cached payment methods from database
      let methods = await storage.getActivePaymentMethods();
      
      // If no cached methods, fetch from MyFatoorah and cache them
      if (methods.length === 0) {
        console.log("No cached payment methods found, fetching from MyFatoorah...");
        const myfatoorahMethods = await myfatoorahService.getPaymentMethods(100, 'SAR');
        
        // Convert to our database format
        const dbMethods = myfatoorahMethods.map(method => ({
          myfatoorahId: method.PaymentMethodId,
          ...
          serviceCharge: method.ServiceCharge?.toString() || "0",
          currencyIso: method.CurrencyIso,
          isActive: true,
        }));
        
        // Save to database
        await storage.savePaymentMethods(dbMethods);
        
        // Get the saved methods
        methods = await storage.getActivePaymentMethods();
      }
  ...
  app.post("/api/payments/create-payment", requireAuth, async (req, res) => {
    try {
      const { invoiceId, paymentMethodId, returnUrl } = req.body;
      ...
      const booking = await storage.getBookingWithDetails(invoice.bookingId);
      if (!booking) {
        return res.status(404).json({ message: "Booking not found" });
      }
      ...
      const callbackUrl = returnUrl || `${baseUrl}/payment-success`;
      ...
      const paymentResponse = await myfatoorahService.createPayment({
        invoiceId: invoice.id,
        ...
        errorUrl: errorUrl,
        language: req.headers['accept-language']?.includes('en') ? 'en' : 'ar',
        items: invoice.invoiceItems ? JSON.parse(invoice.invoiceItems).map((item: any) => ({
          ItemName: item.name,
  ...
  app.get("/api/payments/callback/:transactionId", async (req, res) => {
    try {
      ...
      console.log(`Processing payment callback for transaction ${transactionId}, payment ${paymentId}`);
      
      // Get transaction record
      const transaction = await storage.getPaymentTransaction(parseInt(transactionId));
      if (!transaction) {
        console.error(`Transaction ${transactionId} not found`);
        return res.redirect('/payment-error?error=transaction_not_found');
      }
      ...
      const updates: Partial<PaymentTransaction> = {
        gatewayStatus: statusResponse.Data.InvoiceStatus,
      ...
      if (statusResponse.Data.InvoiceStatus === 'Paid') {
        ...
        if (statusResponse.Data.InvoiceTransactions?.length > 0) {
          const lastTransaction = statusResponse.Data.InvoiceTransactions[0];
          updates.transactionId = lastTransaction.TransactionId;
          updates.referenceId = lastTransaction.ReferenceId;
        }
      } else if (statusResponse.Data.InvoiceStatus === 'Failed' || statusResponse.Data.InvoiceStatus === 'Cancelled') {
        updates.status = statusResponse.Data.InvoiceStatus.toLowerCase();
      }
      
      await storage.updatePaymentTransaction(transaction.id, updates);
      
      // Redirect based on payment status
      if (statusResponse.Data.InvoiceStatus === 'Paid') {
        res.redirect(`/payment-success?transaction=${transactionId}`);
      } else {
        res.redirect(`/payment-error?transaction=${transactionId}&status=${statusResponse.Data.InvoiceStatus}`);
      }
      
    } catch (error: any) {
      console.error("Payment callback error:", error);
      res.redirect('/payment-error?error=callback_failed');
    }
  });

  // Get payment status
  app.get("/api/payments/status/:transactionId", requireAuth, async (req, res) => {
    try {
      const { transactionId } = req.params;
      ...
      // If payment is still pending, check with MyFatoorah
      if (transaction.status === 'pending' && transaction.myfatoorahPaymentId) {
        try {
          const statusResponse = await myfatoorahService.getPaymentStatus({
            Key: transaction.myfatoorahPaymentId,
            KeyType: 'PaymentId'
          });
          
          // Update transaction if status changed
          if (statusResponse.Data.InvoiceStatus !== transaction.gatewayStatus) {
            const updates: Partial<PaymentTransaction> = {
              gatewayStatus: statusResponse.Data.InvoiceStatus,
              gatewayResponse: statusResponse,
            };
            ...
            const updatedTransaction = await storage.getPaymentTransaction(parseInt(transactionId));
            return res.json(updatedTransaction);
          }
        } catch (error: any) {
          console.error("Failed to check payment status with MyFatoorah:", error);
  ...
  app.get("/api/payments/invoice/:invoiceId", requireAuth, async (req, res) => {
    try {
      ...
      const transactions = await storage.getInvoicePaymentTransactions(parseInt(invoiceId));
