            ...
            <div className="bg-white p-8 rounded-lg border shadow-sm">
              ...
              <div className="text-center mb-8 border-b pb-6">
                <h1 className="text-3xl font-bold text-purple-800 mb-2">
                  {language === 'ar' ? 'عيادة بيطرية متنقلة' : 'VETS VAN Mobile Clinic'}
                </h1>
                <p className="text-gray-600">
                  {language === 'ar' ? 
                    'خدمات بيطرية متخصصة في موقعك' : 
                    'Professional Veterinary Services at Your Location'
                  }
                </p>
              </div>
              ...
              <div className="flex justify-between items-start mb-8">
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    <p><strong>{language === 'ar' ? 'رقم الفاتورة:' : 'Invoice No:'}</strong> {invoiceStatus?.invoiceNumber || `Vets${String(booking?.id).padStart(8, '0')}`}</p>
                    <p><strong>{language === 'ar' ? 'التاريخ:' : 'Date:'}</strong> {new Date().toLocaleDateString(language === 'ar' ? 'ar-SA' : 'en-US')}</p>
                    <p><strong>{language === 'ar' ? 'الطبيب:' : 'Doctor:'}</strong> {doctorInfo?.name || 'Dr. VETS VAN'}</p>
                    <p><strong>{language === 'ar' ? 'رمز العيادة:' : 'Clinic Code:'}</strong> {doctorInfo?.vetsvanCode || 'VETS001'}</p>
                  </div>
                </div>
                
                {/* Customer Info */}
                <div className="text-right" style={{ textAlign: language === 'ar' ? 'right' : 'left' }}>
                  <h3 className="text-lg font-semibold mb-3 text-gray-800">
                    {language === 'ar' ? 'بيانات العميل' : 'Customer Details'}
                  </h3>
                  <div className="space-y-1 text-sm">
                    <p><strong>{language === 'ar' ? 'الاسم:' : 'Name:'}</strong> {booking?.customerName}</p>
                    <p><strong>{language === 'ar' ? 'الهاتف:' : 'Phone:'}</strong> {booking?.customerPhone}</p>
                    {booking?.customerEmail && (
                      <p><strong>{language === 'ar' ? 'البريد:' : 'Email:'}</strong> {booking.customerEmail}</p>
                    )}
                    <p><strong>{language === 'ar' ? 'تاريخ الموعد:' : 'Appointment:'}</strong> {booking?.appointmentDate} {booking?.appointmentTime}</p>
                  </div>
                </div>
              </div>
                    ...
                    {booking.pets.map((pet, index) => (
                      <div key={pet.id} className="bg-gray-50 p-4 rounded-lg">
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <p><strong>{language === 'ar' ? 'الاسم:' : 'Name:'}</strong> {pet.name}</p>
                            <p><strong>{language === 'ar' ? 'النوع:' : 'Type:'}</strong> {pet.type}</p>
                          </div>
                          <div>
                            <p><strong>{language === 'ar' ? 'العمر:' : 'Age:'}</strong> {pet.ageYear}Y {pet.ageMonth}M {pet.ageDay}D</p>
                          </div>
                        </div>
                      </div>
                    ))}
                    ...
                    <tbody>
                      {invoiceItems.map((item, index) => (
                        <tr key={item.id}>
                          ...
                          <td className="border border-gray-300 px-3 py-2 text-center">{item.unitPrice.toFixed(2)}</td>
                          ...
                          <td className="border border-gray-300 px-3 py-2 text-center">{item.vatAmount.toFixed(2)}</td>
                          <td className="border border-gray-300 px-3 py-2 text-center font-semibold">{item.totalAfterVat.toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  <div className="bg-gray-50 p-6 rounded-lg border">
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        ...
                        <span>{subtotal.toFixed(2)} {language === 'ar' ? 'ريال' : 'SAR'}</span>
                      </div>
                      {totalDiscountAmount > 0 && (
                        <div className="flex justify-between text-green-600">
                          <span>{language === 'ar' ? 'الخصم:' : 'Discount:'}</span>
                          <span>-{totalDiscountAmount.toFixed(2)} {language === 'ar' ? 'ريال' : 'SAR'}</span>
                        </div>
                      )}
                      <div className="flex justify-between">
                        <span>{language === 'ar' ? 'ضريبة القيمة المضافة (15%):' : 'VAT (15%):'}</span>
                        <span>{taxAmount.toFixed(2)} {language === 'ar' ? 'ريال' : 'SAR'}</span>
                      </div>
                      <div className="border-t pt-3">
                        <div className="flex justify-between text-xl font-bold">
                          <span>{language === 'ar' ? 'الإجمالي النهائي:' : 'Total Amount:'}</span>
                          <span>{finalTotal.toFixed(2)} {language === 'ar' ? 'ريال' : 'SAR'}</span>
                        </div>
                      </div>
                      ...
                      <div className="border-t pt-3 mt-4">
                        <div className="flex justify-between text-green-600 font-semibold">
                          ...
                          <span>{totalPaid.toFixed(2)} {language === 'ar' ? 'ريال' : 'SAR'}</span>
                        </div>
                        <div className="flex justify-between text-red-600 font-semibold">
                          <span>{language === 'ar' ? 'المتبقي:' : 'Balance:'}</span>
                          <span>{remainingBalance.toFixed(2)} {language === 'ar' ? 'ريال' : 'SAR'}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    {invoicePayments.map((payment, index) => (
                      <div key={payment.id} className="flex justify-between items-center bg-gray-50 p-3 rounded">
                        ...
                        <span className="font-semibold">{parseFloat(payment.amount).toFixed(2)} {language === 'ar' ? 'ريال' : 'SAR'}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="text-center mt-12 pt-6 border-t text-sm text-gray-600">
                <p>{language === 'ar' ? 'شكراً لثقتكم بخدماتنا' : 'Thank you for choosing our services'}</p>
                <p className="mt-2">
                  {language === 'ar' ? 
                    'للاستفسارات: info@vetsvan.com | 966-123-456-789' : 
                    'For inquiries: info@vetsvan.com | +966-123-456-789'
                  }
                </p>
              </div>
            </div>
